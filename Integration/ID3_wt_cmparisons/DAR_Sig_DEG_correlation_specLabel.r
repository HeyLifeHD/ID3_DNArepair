...
#output
output.dir <- "/home/heyj/c010-datasets/Internal/2018-Ali/ATAC/200312_ID3_IRcomp/Integration/specLabel"
...
#merge data
goi <- c("EXO1", "RBBP8","FANCM", "BRCA2", "RAD51", "POLQ", "RCF4", "XAB2")
merged <- list()
for(i in names(DAR_list_complete_sign)){
    temp <- inner_join(as.data.frame(DAR_list_complete_sign[[i]]),as.data.frame(DEG_results_list[[i]]), by="ENSEMBL")
    temp$Col <- NA
...
g[[i]] <- ggscatter(merged_sub[[i]], x = "log2FoldChange", y = "Fold",
    ...
    label="Plot",font.label = c(6, "plain","black"), repel = TRUE, #legend.title="P.adjusted",legend = "right",
...
pdf(file.path(output.dir, i , "DAR_RNA_allDARs_allGenes_label.pdf"), width=4,height=4)
    print(g[[i]])
dev.off()

pdf(file.path(output.dir, i , "DAR_RNA_Heat_allDARs_allGenes_label.pdf"), width=6,height=6)
...
dir.create(file.path(output.dir,i))
pdf(file.path(output.dir, i , "DAR_RNA_PromoterDARs_allGenes_label.pdf"), width=4,height=4)
    print(g[[i]])
dev.off()

pdf(file.path(output.dir, i , "DAR_RNA_Heat_PromoterDARs_allGenes_label.pdf"), width=6,height=6)
heatscatter(merged_sub[[i]]$log2FoldChange, merged_sub[[i]]$Fold,
    cexplot = .2,
    xlab=paste0("\nGene expression change\n[log2foldchange]","\n",name),
    ylab=paste0("Chromatin accessibility change\n[log2foldchange]","\n",name),
...
dir.create(file.path(output.dir,i))
pdf(file.path(output.dir, i , "DAR_RNA_allDARs_allSigGenes_label.pdf"), width=4,height=4)
    print(g[[i]])
dev.off()

pdf(file.path(output.dir, i , "DAR_RNA_Heat_allDARs_allSigGenes_label.pdf"), width=6,height=6)
heatscatter(merged_sub[[i]]$log2FoldChange, merged_sub[[i]]$Fold,
    cexplot = .2,
    xlab=paste0("\nGene expression change\n[log2foldchange]","\n",name),
    ylab=paste0("Chromatin accessibility change\n[log2foldchange]","\n",name),
...
pdf(file.path(output.dir, i , "DAR_RNA_Heat_PromoterDARs_allSigGenes_label.pdf"), width=6,height=6)
heatscatter(merged_sub[[i]]$log2FoldChange, merged_sub[[i]]$Fold,
    cexplot = .2,
    xlab=paste0("\nGene expression change\n[log2foldchange]","\n",name),
    ylab=paste0("Chromatin accessibility change\n[log2foldchange]","\n",name),
...
dir.create(file.path(output.dir,i))
pdf(file.path(output.dir, i , "DAR_RNA_allDARs_allSigFCGenes_label.pdf"), width=4,height=4)
    print(g[[i]])
dev.off()

pdf(file.path(output.dir, i , "DAR_RNA_Heat_allDARs_allSigFCGenes_label.pdf"), width=6,height=6)
heatscatter(merged_sub[[i]]$log2FoldChange, merged_sub[[i]]$Fold,
    cexplot = .2,
    xlab=paste0("\nGene expression change\n[log2foldchange]","\n",name),
    ylab=paste0("Chromatin accessibility change\n[log2foldchange]","\n",name),
...
pdf(file.path(output.dir, i , "DAR_RNA_Heat_PromoterDARs_allSigFCGenes_label.pdf"), width=6,height=6)
heatscatter(merged_sub[[i]]$log2FoldChange, merged_sub[[i]]$Fold,
    cexplot = .2,
    xlab=paste0("\nGene expression change\n[log2foldchange]","\n",name),
    ylab=paste0("Chromatin accessibility change\n[log2foldchange]","\n",name),
...
temp <- merged$"ID3-ko_Irr_vs_wt_Irr"
temp <-temp[order(abs(temp$log2FoldChange), decreasing=TRUE),]
head(temp$SYMBOL,20)

temp <- merged$"ID3-ko_unIrr_vs_wt_unIrr"
temp <-temp[order(abs(temp$log2FoldChange), decreasing=TRUE),]
head(temp$SYMBOL,20)

temp <- merged$"ID3-ko_Irr_vs_wt_Irr"
temp <-temp[order(abs(temp$Fold), decreasing=TRUE),]
head(temp$SYMBOL,20)
