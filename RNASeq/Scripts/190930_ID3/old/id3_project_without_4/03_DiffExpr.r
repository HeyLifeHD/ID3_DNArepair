...
annovst <- as.data.frame(colData(rld))[, c("genotype", "irradiation_treatment", "Replicate")] 
color_genotype<-randomColor(length(unique(colData(rld)$genotype)))
...
for (i in names(genes2plot)){
  ...
  #annorow <- ifelse(is.na(plot$symbol),rownames(plot), plot$symbol)
  annorow <- rownames(plot)
  heat[[i]]<- pheatmap(plot[, c(rownames(anno[anno$group==as.character(contrasts[1,i]),]), rownames(anno[anno$group==as.character(contrasts[2,i]),]))], 
...
for (i in names(DEG_results_list)){
  ...
  GO[[i]] <- enrichGO(rownames(temp), OrgDb = org.Hs.eg.db,keyType="ENSEMBL",  
...
for (i in names(GO)){
  plot[[i]] <- dotplot(GO[[i]],showCategory = 20)
  pdf(file.path(PostDE.dir,i,"GO_MF_dotplot.pdf"), width=12)
  print(plot[[i]])
  dev.off()
}
saveRDS(GO, file.path(PostDE.dir, "GO_MF.rds"))
...
lapply(DEG_results_list, function(x){
    ...
    print(nrow(x[which(x$padj < 0.05 & x$log2FoldChange >1),]))
    print(nrow(x[which(x$padj < 0.05 & x$log2FoldChange <(-1)),]))
