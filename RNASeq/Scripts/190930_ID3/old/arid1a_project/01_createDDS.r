...
#load data
counts_list<- readRDS(file.path(data_all.dir , "counts_list.rds"))
counts <- counts_list[["arid1a_project"]]
sample_anno_list <- readRDS(file.path(data.dir, "sample_anno_list.rds"))
sample_anno <- sample_anno_list[["arid1a_project"]]
sample_anno$Replicate <- as.factor(sample_anno$Replicate )
#create dds

dds <- DESeqDataSetFromMatrix(countData = counts, 
                              colData = sample_anno, 
                              design = ~ Replicate+Plattform +group )
