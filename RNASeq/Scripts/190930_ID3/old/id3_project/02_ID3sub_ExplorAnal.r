...
saveRDS(vst_genes,file = file.path(results.dir,"ID3_sub_vst_genes.rds"))
...
pdf(file.path(PreDE.dir, "ID3sub_PCA12_genes.pdf"), height = 7, width = 10)
...
pdf(file.path(PreDE.dir, "ID3sub_PCA23_genes.pdf"), height = 7, width = 10)
...
pdf(file.path(PreDE.dir, "ID3sub_Clustering_vst_genes.pdf"), height = 7, width = 10)
...
saveRDS(vst_genes_b,file =file.path(results.dir, "ID3sub_vst_genes_b_replicate.rds"))
...
pdf(file.path(PreDE.dir, "ID3sub_PCA12_afB.pdf"), height = 7, width = 10)
...
pdf(file.path(PreDE.dir, "ID3sub_PCA23_afB.pdf"), height = 7, width = 10)
...
library(limma)
vst_genes <- vst(dds_sub)
BR<-removeBatchEffect(assay(vst_genes), batch=colData(vst_genes)$Replicate,batch2=colData(vst_genes)$date_sequenced)
vst_genes_b <- SummarizedExperiment(assays =BR , colData=colData(vst_genes))
vst_genes_b <- DESeqTransform(vst_genes_b)
saveRDS(vst_genes_b,file =file.path(results.dir, "ID3sub_vst_genes_b_rep_SeqDate.rds"))
...
pdf(file.path(PreDE.dir, "ID3sub_PCA12_afB_rep_SeqDate.pdf"), height = 7, width = 10)
ggscatter(pcaDatarld, x="PC1", y="PC2",
          color = "group.1", shape = "Replicate", legend = "right",
          ellipse = F ,mean.point = FALSE,repel=TRUE,
          star.plot = F,  xlab=(paste0("PC2: ", percentvarrld.alt[1], "% variance")), ylab=(paste0("PC3: ", percentvarrld.alt[2], "% variance")) )
...
pdf(file.path(PreDE.dir, "ID3sub_PCA23_afB_rep_SeqDate.pdf"), height = 7, width = 10)
ggscatter(pcaDatarld.alt, x="PC2", y="PC3",
          color = "group.1", shape = "Replicate", legend = "right",
          ellipse = F ,mean.point = FALSE,repel=TRUE,
          star.plot = F,  xlab=(paste0("PC2: ", percentvarrld.alt[1], "% variance")), ylab=(paste0("PC3: ", percentvarrld.alt[2], "% variance")) )
