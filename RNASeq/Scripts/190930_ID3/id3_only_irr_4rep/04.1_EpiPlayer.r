...
DEG_results_list_sub_sig <- lapply(DEG_results_list_sub, function(x){
    x <- x[which(x$padj < 0.05 & abs(x$log2FoldChange)>.05),]
    x
})
...
for (i in names(DEG_results_list_sub_sig)){
    ...
    pdf(file.path(PostDE.dir,i,"Bar_sig_fc_EpiPlayer.pdf"), width=35)
    print(ggbarplot(DEG_results_list_sub_sig[[i]], x="Symbol", y="log2FoldChange",fill="Change",palette=col, sort.val="desc")
    + rremove("xlab")+ rremove("legend")+rotate_x_text(angle = 45))
    dev.off()
}
