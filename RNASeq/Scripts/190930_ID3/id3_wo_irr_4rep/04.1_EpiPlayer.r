...
#get propper annotation
epi_human <- epi_human[,c("gene.name", "group", "subgroup")]
colnames(epi_human)<- c("Symbol", "group", "subgroup")
rownames(epi_human)<- epi_human$Symbol 
epi_human$symbol <- epi_human$Symbol
...
#remove dna repair player from list
epi_human_sub <- epi_human[epi_human$group != "DNA repair",  ]
dim(epi_human_sub)
#subset list
lapply(DEG_results_list,function(x)dim(x))
DEG_results_list_sub <- lapply(DEG_results_list, function(x){
    x <- x[which(rownames(x) %in% epi_human_sub$ENSEMBL),]
    x <- left_join(x, epi_human_sub)
    rownames(x)<- x$ensembl
    x
})
lapply(DEG_results_list_sub,function(x)dim(x))

#subset based on fc and sign.
...
color_genotype<-color_genotype<-RColorBrewer::brewer.pal(n = length(unique(colData(rld_b)$genotype)), name = 'Dark2')
...
for (i in names(rld_sub_sig)){
    heat[[i]]<- pheatmap(rld_sub_sig[[i]][,rownames(anno)], scale="row", show_colnames=F,#labels_row=annorow,
                       annotation_col=as.data.frame(annovst),fontsize_row=2,
                       annotation_colors=anno_colors, 
                       clustering_distance_rows="correlation",
                       show_rownames=T) 
    pdf(file.path(PostDE.dir,i,"Heat_sig_EpiPlayer_sub.pdf"))
    print(heat[[i]])
    dev.off() 
    ...
    heat[[i]]<- pheatmap(rld_sub_sig[[i]][, c(rownames(anno[anno$group==as.character(contrasts[1,i]),]), rownames(anno[anno$group==as.character(contrasts[2,i]),]))],
                       ...
                       show_rownames=T) 
    pdf(file.path(PostDE.dir,i,"Heat_sig_EpiPlayer_sub_subset.pdf"))
    print(heat[[i]])
    dev.off() 
}
...
for (i in names(DEG_results_list_sub_sig_onlyFDR)){
    ...
    pdf(file.path(PostDE.dir,i,"Bar_sig_EpiPlayer.pdf"), width=25)
...
#same for fc and fdr cutoff
#plot heatmaps of those genes
rld_expr <- assay(rld_b)
rld_sub_sig <- lapply(DEG_results_list_sub_sig, function(x){
    x <- rld_expr[which(rownames(rld_expr) %in% rownames(x)),]
})
lapply(rld_sub_sig, function(x)dim(x))
...
for (i in names(rld_sub_sig)){
    heat[[i]]<- pheatmap(rld_sub_sig[[i]][,rownames(anno)], scale="row", show_colnames=F,#labels_row=annorow,
                       annotation_col=as.data.frame(annovst),fontsize_row=5,
                       annotation_colors=anno_colors, 
                       clustering_distance_rows="correlation",
                       show_rownames=T) 
    pdf(file.path(PostDE.dir,i,"Heat_sig_fc_EpiPlayer_sub.pdf"))
    print(heat[[i]])
    dev.off() 
    ...
    heat[[i]]<- pheatmap(rld_sub_sig[[i]][, c(rownames(anno[anno$group==as.character(contrasts[1,i]),]), rownames(anno[anno$group==as.character(contrasts[2,i]),]))],
                       ...
                       annotation_colors=anno_colors, clustering_distance_rows="correlation",
                       show_rownames=T) 
    pdf(file.path(PostDE.dir,i,"Heat_sig_fc_EpiPlayer_sub_subset.pdf"))
    print(heat[[i]])
    dev.off() 
}
...
for (i in names(DEG_results_list_sub_sig)){
    DEG_results_list_sub_sig[[i]]$Symbol <- DEG_results_list_sub_sig[[i]]$symbol
    DEG_results_list_sub_sig[[i]]<-     DEG_results_list_sub_sig_onlyFDR[[i]][order(DEG_results_list_sub_sig[[i]]$log2FoldChange, decreasing=TRUE),]
    DEG_results_list_sub_sig[[i]]$Change <- ifelse(DEG_results_list_sub_sig[[i]]$log2FoldChange >0, "blue", "red")
    pdf(file.path(PostDE.dir,i,"Bar_sig_fc_EpiPlayer.pdf"), width=9)
    print(ggbarplot(DEG_results_list_sub_sig[[i]], x="Symbol", y="log2FoldChange",fill="Change",palette=col)
    + rremove("xlab")+ rremove("legend")+rotate_x_text(angle = 45))
    dev.off()
}
