...
#Directories
base.dir <- "/home/heyj/c010-datasets/Internal/Dieter/TALL_ACT"
analysis.dir <- file.path(base.dir, "YY_comp_analysis", "DiffBind")
...
#Create Sample sheet
#get files 
bam_files <- list.files(base.dir, pattern="tn5correct.bam$", full.names=TRUE, recursive=TRUE)
bam_files <- bam_files[grep("P-P6-2_", bam_files, invert=TRUE)]
peak_files <- list.files(base.dir, pattern="_tn5_center_73bp.macs2_peaks.broadPeak$", full.names=TRUE, recursive=TRUE)
...
for (i in 1:nrow(contrasts)){
    ...
    print(dba.plotMA(dataset,contrast=i, method= DBA_EDGER_BLOCK))
    dev.off()   
    #volcano plot
    pdf(file.path(analysis.dir, "Volcanos",  paste0("Vol_", contrasts[i, 4], ".pdf")))
    print(dba.plotVolcano(dataset,contrast=i, method= DBA_EDGER_BLOCK))
    dev.off()
    }
    if(length(temp)>1){
    #annotate DE regions
