...
#only arid1a
annovst <- as.data.frame(colData(vst_repeats)[anno$sample_name,])
annovst <- annovst[annovst$genotype %in% c("WT", "ARID1A") & annovst$tamoxifen_treatment == "untreated" & annovst$hdac_treatment == "untreated" &
annovst$irradiation_treatment == "untreated", ]
vst_as <- assay(vst_repeats)
vst_LTR <- vst_as[grep("LTR12",rownames(vst_as)),rownames(annovst)]
#Plot LTR12
annovst$replicate <- as.factor(annovst$replicate)
annovst <- annovst[, c("genotype", "tamoxifen_treatment","hdac_treatment", "replicate", "irradiation_treatment" )]
pheatmap(vst_LTR, 
                ...
                cluster_rows=F,fontsize = 5,
                #color=col_palette,
                filename= file.path(PreDE.dir, "ARID1A_WT_repName_LTR_Heatmap_Scaled.pdf"),
                border_color = "grey"
                #cellwidth=5,
)
...
#only id3
annovst <- as.data.frame(colData(vst_repeats)[anno$sample_name,])
annovst <- annovst[annovst$genotype %in% c("WT", "ID3") & annovst$tamoxifen_treatment == "untreated" & annovst$hdac_treatment == "untreated" &
annovst$irradiation_treatment == "untreated", ]
vst_as <- assay(vst_repeats)
vst_LTR <- vst_as[grep("LTR12",rownames(vst_as)),rownames(annovst)]
#Plot LTR12
...
pheatmap(vst_LTR, 
                ...
                cluster_rows=F,fontsize = 5,
                #color=col_palette,
                filename= file.path(PreDE.dir, "ID3_WT_repName_LTR_Heatmap_Scaled.pdf"),
                border_color = "grey"
                #cellwidth=5,
)
...
tsne <- Rtsne(t(unique(assay(vst_repeats))), dims = 2, perplexity=11, verbose=TRUE, max_iter = 500)
colors = rainbow(length(unique( colData(dds_repeats)$group)))
names(colors) = unique( colData(dds_repeats)$group)

pdf(file.path(PreDE.dir, "tsne_repeats.pdf"), height = 7, width = 10)
plot(tsne$Y, t='n', main="tsne")
text(tsne$Y, labels=rownames(t(unique(assay(vst_repeats)))), col =colors)
dev.off()