...
idx <- rownames(anno[anno$genotype!="MDC1",])
anno <-anno[rownames(anno) %in% idx,]
...
#subset repeat data
count_repeats <- count_repeats[, rownames(anno)]
#sum up per rep name
...
#create df
count_repeats <- counts_sum_repName[,rownames(anno)]
...
#subset counts
counts_genes <- counts[,rownames(anno)]
