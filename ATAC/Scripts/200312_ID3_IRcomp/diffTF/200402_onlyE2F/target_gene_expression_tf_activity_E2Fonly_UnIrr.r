...
tf_binding <- fread("/home/heyj/c010-datasets/Internal/2018-Ali/ATAC/200312_ID3_IRcomp/analysis/diffTF/200402_output_onlyE2F_unIrr_anal/FINAL_OUTPUT/extension100/wt_unIrrvsID3-ko_unIrr.all.allMotifs.tsv.gz")
...
diffTF_res_unIrr <- fread("/home/heyj/c010-datasets/Internal/2018-Ali/ATAC/200312_ID3_IRcomp/analysis/diffTF/200402_output_onlyE2F_unIrr_anal/FINAL_OUTPUT/extension100/wt_unIrrvsID3-ko_unIrr.all.summary.tsv.gz")

#subset significant results
diffTF_res_unIrr_sub <- diffTF_res_unIrr[which(diffTF_res_unIrr$pvalueAdj < 0.05),]
dim(diffTF_res_unIrr_sub)
tf_binding_sub <- tf_binding[tf_binding$TF %in% diffTF_res_unIrr_sub$TF]
...
ID3KO_unIrr_vs_WT_unIrr <- DEG_results_list$ID3_Irr.TREATED_Tam.UNTREATED_Hdac.UNTREATED_vs_WT_Irr.TREATED_Tam.UNTREATED_Hdac.UNTREATED
...
tf_target_gene_promoter_list[[i]] <- mean(ID3KO_unIrr_vs_WT_unIrr[ID3KO_unIrr_vs_WT_unIrr$ensembl %in% tf_binding_sub_split_anno_promoter[[i]]$ENSEMBL,]$log2FoldChange, na.rm=  TRUE) 
}
...
merge <- inner_join(tf_target_gene_promoter, diffTF_res_unIrr, by="TF")
...
pdf(file.path("/home/heyj/c010-datasets/Internal/2018-Ali/ATAC/200312_ID3_IRcomp/analysis/diffTF/200402_output_onlyE2F_unIrr_anal/FINAL_OUTPUT/",  "MeanTargetGeneExpression_TFactivity_correlation_onlyPromoter_unIrr.pdf"), width=7,height=7)
print(g)
dev.off()
...
for(i in names(tf_binding_sub_split_anno_promoter)){
tf_target_gene_promoter_list_comp[[i]] <-ID3KO_unIrr_vs_WT_unIrr[ID3KO_unIrr_vs_WT_unIrr$ensembl %in% tf_binding_sub_split_anno_promoter[[i]]$ENSEMBL,]
tf_target_gene_promoter_list_comp_sub[[i]] <- tf_target_gene_promoter_list_comp[[i]][which(tf_target_gene_promoter_list_comp[[i]]$padj < 0.1),]
}
tf_target_gene_promoter_list_comp_sub
