...
#DEG_results_list<- readRDS(file.path(PostDE.dir, "DEG_results_group_list_combinedMG.rds"))
#DEG_results_list$"TAM_Tumor_vs_NeuT_MG_healthy"$log2FoldChange <- -(DEG_results_list$"TAM_Tumor_vs_NeuT_MG_healthy"$log2FoldChange )
#DEG_results_list$"BMDM_Tumor_vs_BMDM_healthy"$log2FoldChange <- -(DEG_results_list$"BMDM_Tumor_vs_BMDM_healthy"$log2FoldChange )
#names(DEG_results_list) <- c("BMDM_healthy_vs_BMDM_tumor", "BMDM_tumor_vs_TAM_tumor", "BMDM_healthy_vs_TAM_tumor", "TAM_healthy_vs_TAM_tumor")
...
names_knownRes_sub <- sapply(names_knownRes, function(x) paste0(x[12], "_", x[13]))
names(knownRes) <- names_knownRes_sub
homer_list <- mclapply(knownRes, function(x)read_homer_output(x),mc.cores=2)

#subdevide
DAR_list <- list()
for(i in unique(sapply(names_knownRes, function(x) x[12]))){
...
for (i in names(DAR_list)){
    ...
    DAR_list_sub_plot[[i]] <- DAR_list_sub_plot[[i]][grep("E2F", DAR_list_sub_plot[[i]]$MotifName), ]
    
    pdf(file.path(analysis.dir, "homer", i,paste0("HomerBubble_E2F.pdf")))
    print(bubblePlot(DAR_list_sub_plot[[i]])+ggtitle(i))
    dev.off()

}

#perform common plotting
DAR_list_oI <- list(DAR_list$"ID3-ko_Irr_vs_wt_Irr"$closed, DAR_list$"ID3-ko_Irr_vs_wt_Irr"$open,
   DAR_list$"ID3-ko_unIrr_vs_wt_unIrr"$closed, DAR_list$"ID3-ko_unIrr_vs_wt_unIrr"$open )
names(DAR_list_oI)<- c("ID3-ko_Irr_vs_wt_Irr_closed", "ID3-ko_Irr_vs_wt_Irr_open",
    "ID3-ko_unIrr_vs_wt_unIrr_closed", "ID3-ko_unIrr_vs_wt_unIrr_open")
DAR_list_oI <- list(DAR_list_oI)
names(DAR_list_oI)<-"combined"

#all E2F motifs
DAR_list_sub <- list()
DAR_list_sub_sig <- list()
DAR_list_sub_plot<-list()
for (i in names(DAR_list_oI)){
    DAR_list_sub[[i]]<- lapply(DAR_list_oI[[i]], function(x){
    x$known_motif_table$MotifName <- sapply(strsplit(x$known_motif_table$motif_name ,"(", fixed=TRUE),`[`, 1)
    x$known_motif_table$MotifName <- sapply(strsplit(x$known_motif_table$MotifName ,"/", fixed=TRUE),`[`, 1)
    x$known_motif_table$MotifName <- toupper(x$known_motif_table$MotifName)
    #result.df<- DEG_results_list[[i]]
    ...
    DAR_list_sub_plot[[i]] <- DAR_list_sub_plot[[i]][grep("E2F", DAR_list_sub_plot[[i]]$MotifName), ]
    dir.create(file.path(analysis.dir, "homer", i))
    pdf(file.path(analysis.dir, "homer", i,paste0("HomerBubble_E2F.pdf")))
    print(bubblePlot(DAR_list_sub_plot[[i]])+ggtitle(i))
    dev.off()
...
#all comparisons
#perform common plotting
DAR_list_oI <- list(DAR_list$"ID3-ko_Irr_vs_ID3-ko_unIrr"$closed, DAR_list$"ID3-ko_Irr_vs_ID3-ko_unIrr"$open,
    DAR_list$"wt_Irr_vs_wt_unIrr"$closed, DAR_list$"wt_Irr_vs_wt_unIrr"$open,
    DAR_list$"ID3-ko_Irr_vs_wt_Irr"$closed, DAR_list$"ID3-ko_Irr_vs_wt_Irr"$open,
    DAR_list$"ID3-ko_unIrr_vs_wt_unIrr"$closed, DAR_list$"ID3-ko_unIrr_vs_wt_unIrr"$open )
names(DAR_list_oI)<- c("ID3-ko_Irr_vs_ID3-ko_unIrr_closed", "ID3-ko_Irr_vs_ID3-ko_unIrr_open",
    "wt_Irr_vs_wt_unIrr_closed", "wt_Irr_vs_wt_unIrr_open",
    "ID3-ko_Irr_vs_wt_Irr_closed", "ID3-ko_Irr_vs_wt_Irr_open",
    "ID3-ko_unIrr_vs_wt_unIrr_closed", "ID3-ko_unIrr_vs_wt_unIrr_open")
DAR_list_oI <- list(DAR_list_oI)
names(DAR_list_oI)<-"combined"
...
for (i in names(DAR_list_oI)){
    ...
    DAR_list_sub_plot[[i]] <- DAR_list_sub_plot[[i]][grep("E2F", DAR_list_sub_plot[[i]]$MotifName), ]
    dir.create(file.path(analysis.dir, "homer", i))
    pdf(file.path(analysis.dir, "homer", i,paste0("HomerBubble_allComparisons_E2F.pdf")),width=9)
    print(bubblePlot(DAR_list_sub_plot[[i]])+ggtitle(i))
    dev.off()
