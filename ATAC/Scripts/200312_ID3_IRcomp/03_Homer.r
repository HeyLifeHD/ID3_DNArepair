...
write.table(data.frame(chr=DE_list_complete_df$seqnames, 
        ...
        file.path(analysis.dir,"homer", "all_peaks_df.txt"),
        ,sep="\t", quote=FALSE, row.names=FALSE, col.names = FALSE)

#export lists for open 
#annotate peaks
DE_list_complete_sign_fc_df <- lapply(DE_list_complete_sign_fc, function(x){
    x <- as.data.frame(x)
...
for(i in names(DE_list_complete_sign_fc)){
    dars_final_df[[i]] <- DE_list_complete_sign_fc_df[[i]]
    dars_final_df[[i]]<- dars_final_df[[i]][which(dars_final_df[[i]]$Fold >1),]
    dir.create(file.path(analysis.dir, "homer",i, "open"), recursive=TRUE)
    #open
    write.table(data.frame(chr=dars_final_df[[i]]$seqnames, 
        ...
        file.path(analysis.dir, "homer",i, "open", paste0("dars.bed")),sep="\t", quote=FALSE, row.names=FALSE, col.names = FALSE)
    #closed
    dars_final_df[[i]] <- DE_list_complete_sign_fc_df[[i]]
    dars_final_df[[i]]<- dars_final_df[[i]][which(dars_final_df[[i]]$Fold <(-1)),]
    dir.create(file.path(analysis.dir, "homer",i, "closed"), recursive=TRUE)
    write.table(data.frame(chr=dars_final_df[[i]]$seqnames, 
        start=dars_final_df[[i]]$start, end=dars_final_df[[i]]$end, 
        id=dars_final_df[[i]]$peak_id, notUsed=NA, strand="+"),
        file.path(analysis.dir, "homer",i, "closed", paste0("dars.bed")),sep="\t", quote=FALSE, row.names=FALSE, col.names = FALSE)

}

#run in command line
cd /home/heyj/c010-datasets/Internal/2018-Ali/ATAC/200312_ID3_IRcomp/analysis/DiffBind/homer
