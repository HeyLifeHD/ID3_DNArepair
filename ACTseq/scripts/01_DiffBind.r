...
#Create Sample sheet
#get files 
bam_files <- list.files(data.dir, pattern="_filt_tn5correct.bam$", full.names=TRUE, recursive=TRUE)
bam_files <- bam_files[grep("IgG", bam_files, invert=TRUE)]
peak_files <- list.files(data.dir, pattern="_filt_tn5correct.narrow_peaks.narrowPeak$", full.names=TRUE, recursive=TRUE)
peak_files <- peak_files[grep("IgG", peak_files, invert=TRUE)]
#GET NAMES
file_names <- sapply(strsplit(peak_files, "/", fixed=TRUE), "[", 9)
file_names <- sapply(strsplit(file_names, "_filt", fixed=TRUE), "[", 1)
...
sample_anno <- data.frame("SampleID"=file_names,
    ...
    "Replicate"= paste0("rep",sapply(strsplit(file_names, "_", fixed=TRUE), "[", 4)),
...
dataset <- readRDS(file.path(analysis.dir,"Dataset_peak.rds"))
#correlation Heatmap
...
for (i in 1:nrow(contrasts)){
    ...
    if(i <=2){
        temp <- dba.report(dataset, contrast=i ,method =DBA_EDGER, th=0.05)
    } else{
        temp <- dba.report(dataset, contrast=i ,method =DBA_EDGER, th=0.05)
    }
    if(is.null(temp)) {}
    ...
    print(dba.plotMA(dataset,contrast=i, method= DB.DESeq2.block))
    dev.off()   
    #volcano plot
    pdf(file.path(analysis.dir, "Volcanos",  paste0("Vol_", contrasts[i, 4], ".pdf")))
    print(dba.plotVolcano(dataset,contrast=i, method= DB.DESeq2.block))
    dev.off()
    }
    if(length(temp)>1){
    #annotate DE regions
    ...
    #plot MA
    pdf(file.path(analysis.dir, "MAs",  paste0("MA_", contrasts[i, 4], ".pdf")))
    print(dba.plotMA(dataset,contrast=i, method= DBA_EDGER))
    dev.off()   
    #volcano plot
    pdf(file.path(analysis.dir, "Volcanos",  paste0("Vol_", contrasts[i, 4], ".pdf")))
    print(dba.plotVolcano(dataset,contrast=i, method= DBA_EDGER))
    dev.off()
    #Heatmap
    pdf(file.path(analysis.dir, "Heatmaps",  paste0("Heat_", contrasts[i, 4], ".pdf")))
    print(dba.plotHeatmap(dataset, contrast=i, correlations=FALSE, scale="row", method=DBA_EDGER))
    dev.off()
    #Boxplot
    pdf(file.path(analysis.dir, "Boxplots",  paste0("Box_", contrasts[i, 4], ".pdf")))
    print(dba.plotBox(dataset,contrast=i, method=DBA_EDGER))
    dev.off()
    }
}
...
for (i in 1:nrow(contrasts)){
    #specific for this mixed analysis
    if(i <=2){
        temp <- dba.report(dataset, contrast=i ,method =DBA_EDGER, th=1)
    } else{
        temp <- dba.report(dataset, contrast=i ,method =DBA_EDGER, th=1)
    }    
